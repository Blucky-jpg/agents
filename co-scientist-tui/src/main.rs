...
/// Strip `[[MEMORY_OP:<op>:{json}]]` markers from streaming deltas before
/// they land in the chat log. `TurnDone` parses + strips markers from the
/// final text, but `TurnDelta` carries the raw LLM text — without this
/// scrub the raw marker renders as a plain markdown paragraph in the
/// live log until `TurnDone` arrives (the screenshot showed `memory_add`
/// leaking during streaming).
///
/// The scrubber is a small bracket-buffered state machine:
/// - `Outside` → just text. Pass through.
/// - `MaybeOpen` → one `[` seen. Pass through; if the next char is `[`,
///   move to `Inside`. If anything else, drop back to `Outside` (the
///   `[` was a literal bracket in prose, not a marker opener).
/// - `Inside` → consume until `]]` is seen. Internal `{` / `}` braces are
///   counted (so a JSON body containing nested braces is consumed as one
///   unit). The closing `]]` returns us to `Outside`.
///
/// `carry_in` is the partial-marker tail from the previous delta that
/// must be processed first. We mutate it to the new unconsumed tail when
/// the input ends mid-marker — that way a `[[MEMORY_OP:save:{"scope":"x"`
/// in one delta followed by `,"summary":"y"}]]` in the next delta is
/// scrubbed as a unit.
///
/// Why not use `co_scientist::skill::parse_markers`? That function expects
/// the FULL text in one shot; it returns the cleaned text + parsed markers
...
mod scrub_tests {
    ...
    use super::scrub_markers;

    #[test]
    fn whole_marker_stripped_in_single_delta() {
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "hi [[MEMORY_OP:noop:{}]] bye");
    ...
    fn prefix_only_no_false_positive() {
        ...
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "text [[ not a marker ]] more");
    ...
    fn bare_open_bracket_no_false_positive() {
        ...
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "price [$10] off");
    ...
    fn marker_split_across_two_deltas() {
        ...
        let mut carry = String::new();
        let out1 = scrub_markers(&mut carry, "hello [[MEMORY_OP:save_semantic:{\"scope\":\"x\"");
        ...
        let out2 = scrub_markers(&mut carry, ",\"summary\":\"y\"}]] world");
    ...
    fn nested_json_brace_not_confused() {
        ...
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "before [[MEMORY_OP:t:{\"k\":{\"v\":1}}]] after");
    ...
    fn unterminated_marker_carries_forward() {
        ...
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "before [[MEMORY_OP:save:{\"scope\":");
    ...
    fn markdown_text_with_brackets_passes_through() {
        ...
        let mut carry = String::new();
        let out = scrub_markers(&mut carry, "see [docs](https://example.com) and `[]`");
